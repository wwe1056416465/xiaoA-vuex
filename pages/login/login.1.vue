		...
		<view class="login_box">
			...
			<form>
				...
				<view class="uni-form-item uni-column login_pws margin-b30">
					<view class="with-fun border-b">
						<input type="password" class="uni-input " placeholder="请输入密码" :value="userpws" @input="clearpws" />
	...
	export default {
		...
		methods: {
			...
			clearpws: function(event) {
				this.userpws = event.target.value;
				if (event.target.value.length > 0) {
			...
			loginto() {
				this.isCommit=true;
				console.log(this.isCommit)
				if (this.yuming.trim().length <= 0) {
					uni.showToast({
						icon: 'none',
						title: '请输入域名'
					});
				...
				let yuming=this.yuming
				if(this.yuming.indexOf('https://')==-1){
					yuming='https://'+this.yuming;
				}
				uni.setStorageSync('yuming', yuming);
				this.saveyuming(yuming)
				uni.request({
						url:yuming + '/adminspa/login/login',
						header: {
